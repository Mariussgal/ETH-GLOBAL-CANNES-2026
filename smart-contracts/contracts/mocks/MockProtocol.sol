...
contract MockProtocol {
    ...
    event FeesGenerated(uint256 amount, uint256 timestamp, uint256 feeCount);
    ...
    function _sendFees(uint256 amount) internal {
        ...
        emit FeesGenerated(amount, block.timestamp, feeCount);
    }

    function nextFeeAllowed() external view returns (uint256) {
        return lastFeeTimestamp + FEE_INTERVAL;
    }
